...
type App struct {
	...
	// FAQService backs the knowledge-base ingestion endpoint.
...
func New(ctx context.Context, cfg *config.Config, log *slog.Logger) (*App, error) {
	...
	if err := postgres.SeedFAQ(ctx, pool, log); err != nil {
		pool.Close()
		return nil, fmt.Errorf("bootstrap: seed faq: %w", err)
	}
	...
	tokenUsageRepo := postgres.NewTokenUsageRepository(pool, log)
	feedbackRepo := postgres.NewFeedbackRepository(pool, log)
	publisher := eventlog.New(log)

	completer := llm.NewCompleter(cfg.AI, tokenUsageRepo, log)
	embedder := embedding.NewEmbedder(cfg.AI, log)

	faqService := faq.New(faqRepo, faqRepo, embedder, completer, faq.Config{
		Threshold: cfg.AI.RagThreshold,
		TopK:      cfg.AI.RagTopK,
		MaxTokens: cfg.AI.RagMaxTokens,
		MockOnly:  embedder.UsesMockOnly(),
		Static:    chat.StaticAnswers{},
	}, log)
	...
	replyService := chat.NewReplyService(sessions, ticketRepo, responder, identitySvc, publisher, feedbackRepo, cfg.Session.IdleTimeout, log)
